...
Yes. Full Ethermint EVM with Ethereum JSON-RPC on port 8545 (HTTP) and 8546 (WebSocket). The HTTP interface is available on the public RPC endpoints; WebSocket is currently reachable only on a node you run yourself, and public WebSocket access is not yet enabled. MetaMask, ethers.js, Hardhat, and Foundry all work with Chain ID `2127` (mainnet) or `21270` (testnet).

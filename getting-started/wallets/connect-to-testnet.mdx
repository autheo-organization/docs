...
<Note>
  The RPC URL above is a JSON-RPC endpoint. It accepts `POST` requests from wallets and applications, not browser `GET` requests. Visiting it directly in a browser returns `405 Method Not Allowed`. This is expected behavior and does not indicate a problem with the network.
</Note>
...
You can access the [Autheo Testnet faucet](https://faucet.testnet.autheo.com/).

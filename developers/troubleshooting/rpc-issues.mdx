...
**Fix**:
1. On mainnet, try an alternate endpoint: `https://rpc2.autheo.com` or `https://rpc3.autheo.com`. On testnet, `https://rpc.testnet.autheo.com` is the only public endpoint.
...
<Note>
This applies to a node you run yourself. **Public WebSocket endpoints are not currently offered** on Mainnet or Testnet (see [Network endpoints](/getting-started/network/endpoints)).
</Note>
...
```javascript
function connect() {
  const ws = new ethers.WebSocketProvider("ws://127.0.0.1:8546");

...
- **Use WebSocket subscriptions** for real-time indexing, with HTTP fallback for historical backfills. Public WebSocket access is not yet available (see [Network endpoints](/getting-started/network/endpoints)), so index over HTTP for now or subscribe against your own node.

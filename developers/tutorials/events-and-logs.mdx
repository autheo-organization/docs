...
<Note>
Real-time subscriptions require a WebSocket connection, and **public WebSocket endpoints are not currently offered** on Mainnet or Testnet (see [Network endpoints](/getting-started/network/endpoints)).

Query past events over HTTP with `queryFilter`, or point the provider at the local WebSocket port on a node you run yourself.
</Note>

...
description: "End-to-end guide for running an Autheo Chain validator node on your own infrastructure using Docker or systemd, including network configuration, sync methods, key management, validator registration, and claiming rewards."
keywords: ["self-host", "validator", "node setup", "docker", "systemd", "autheod", "autheo chain", "ghcr", "installation", "claim rewards"]
---
...
<Warning>
RPC (26657), gRPC (9090), REST (1317), and EVM JSON-RPC/WS (8545/8546) should stay bound to `127.0.0.1` on a production validator — see the firewall table in [Security hardening](/nodes/operations/security-hardening), which lists all of these as **Closed**. Only bind them to `0.0.0.0` below if you have your own firewall rules or a sentry architecture in front of this node restricting who can reach these ports. If you don't, skip this section and leave the defaults in place.
</Warning>
...
# Expose RPC to all interfaces (default is 127.0.0.1) — see warning above
...
sudo mkdir -p /home/autheo
sudo cp -r $HOME/.autheo /home/autheo/.autheo
sudo chown -R autheo:autheo /home/autheo
```
...
<Warning>
The `-p` flags below publish each port to all interfaces on the host, equivalent to the `0.0.0.0` binds in [section 3.4](#34-configure-the-node). Per the firewall table in [Security hardening](/nodes/operations/security-hardening), only 26656 (P2P) should be reachable from the internet on a validator. For the others, either drop the `-p` flag (keep the service container-internal only) or bind to localhost explicitly, e.g. `-p 127.0.0.1:26657:26657`.
</Warning>
...
Downloads a recent snapshot from a live rpc and syncs only the remaining blocks.

```bash
TRUST_HEIGHT=$(curl -s https://cosmos-rpc1.autheo.com/block | jq -r '(.result.block.header.height | tonumber / 1000 | floor * 1000)')
TRUST_HASH=$(curl -s "https://cosmos-rpc1.autheo.com/block?height=$TRUST_HEIGHT" | jq -r '.result.block_id.hash')

CONFIG=$HOME/.autheo/config/config.toml
sed -i '/^\[statesync\]/,/^\[/{s/^enable = false/enable = true/}' $CONFIG
sed -i "s|^rpc_servers = .*|rpc_servers = \"https://cosmos-rpc1.autheo.com,https://cosmos-rpc2.autheo.com,https://cosmos-rpc3.autheo.com\"|" $CONFIG
...
Once your validator is `ACTIVE`, two independent reward streams accrue in parallel — neither one claims automatically, and neither expires.

**NFT emission rewards** (from your Sovereign license, via `x/licensedistribution`):

```bash
# Check accrued amount
autheod query emissions license-accrued-rewards <license_id> \
  --node http://localhost:26657

# Claim
autheod tx licensedistribution claim-nft-rewards <license_id> \
  --from <key_name> \
  --keyring-backend file \
  --chain-id autheo_2127-1 \
  --gas auto \
...
```bash
# Check accrued amount (includes commission)
autheod query distribution validator-outstanding-rewards <autheovaloper_address> \
  --node http://localhost:26657

# Claim, including commission
autheod tx distribution withdraw-rewards <autheovaloper_address> \
  --commission \
  --from <key_name> \
  --keyring-backend file \
  --chain-id autheo_2127-1 \
...
Run the same two commands with `docker exec autheo-node ...` and `--home /root/.autheo` for Docker deployments.

See [Staking and rewards](/token/staking-and-rewards) for the full breakdown of both reward streams, rates by license tier, and the global emissions cap.

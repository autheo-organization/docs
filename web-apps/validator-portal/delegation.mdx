---
title: "Delegation"
description: "How to find validators, delegate THEO tokens, and bind Prime or Core licenses in the Autheo Validator Portal."
...
- Use the search bar to find validators by **validator name (moniker)** or **validator address**.
...
> **Order matters:** Delegate your THEO to the validator first, then bind your Prime or Core license to that delegation. The license becomes `ACTIVE` after it is successfully bound.

## Binding a license

After delegating, you can bind a **Prime or Core license to that delegation**. The license becomes associated with the selected validator and moves from `BOUND` to `ACTIVE` status when the transaction succeeds.
...
| Column | Description |
| --- | --- |
| Validator | The validator name (moniker) and operator address |
...
> **Changing validators:** Autheo does not currently support redelegation. To move your THEO to another validator, you must unstake first and wait for the approximately 21-day unbonding period to complete before delegating to the new validator. See [Moving your delegation to another validator](/web-apps/validator-portal/rewards-and-unstaking#moving-your-delegation-to-another-validator).
